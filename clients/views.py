...
from clients.forms import AccountForm, AccountCommentForm
...
def clients_list(request):
    ...
    if name:
        clients_list = Account.objects.filter(name__icontains=name)
    if city:
        clients_list = clients_list.filter(
            billing_address__in=[i.id for i in Address.objects.filter(city__contains=city)])
    if industry:
        clients_list = clients_list.filter(industry__icontains=industry)
    ...
    if request.method == 'POST':
        ...
        if account_form.is_valid() and billing_form.is_valid() and shipping_form.is_valid():
            ...
            if request.POST.get("savenewform"):
                return redirect("clients:new_account")
            else:
