...
from cases.models import Case, UploadFile
from cases.forms import CaseForm, CaseCommentForm, DocumentForm
...
def add_case(request):
    ...
    form = CaseForm(assigned_to=users, account=clients, contacts=contacts)
    ...
    if request.method == 'POST':
        form = CaseForm(request.POST, assigned_to=users, account=clients, contacts=contacts)
        if form.is_valid():
...
def view_case(request, case_id):
    ...
    case_object = UploadFile.objects.all().values()

    # documents_record =  get_object_or_404(
    #     UploadFile.objects.prefetch_related("name_id", "description"), id=name_id)
    # documents = documents_record.cases.all()
...
def edit_case(request, case_id):
    ...
    clients = Account.objects.all()
    contacts = Contact.objects.all()
    form = CaseForm(instance=case_object, assigned_to=users, account=clients, contacts=contacts)
    teams = Team.objects.all()
    teams_list = request.POST.getlist("teams")
    assignedto_list = request.POST.getlist("assigned_to")
    contacts_list = request.POST.getlist("contacts")
    if request.method == 'POST':
        form = CaseForm(
            request.POST, instance=case_object, assigned_to=users, account=clients,
            contacts=contacts
        )
        if form.is_valid():
...
def upload_file(request):
    ...
    return render(request, "cases/upload_file.html", {'form':form, 'clients':cases})
