...
def add_contact(request):
    ...
    form = ContactForm(assigned_to=users, account=clients)
    address_form = BillingAddressForm()
    teams = Team.objects.all()
    assignedto_list = request.POST.getlist('assigned_to')
    teams_list = request.POST.getlist('teams')
    if request.method == 'POST':
        form = ContactForm(request.POST, assigned_to=users, account=clients)
...
def edit_contact(request, pk):
    ...
    clients = Account.objects.filter()
    users = User.objects.filter(is_active=True).order_by('email')
    form = ContactForm(instance=contact_obj, assigned_to=users, account=clients)
    address_form = BillingAddressForm(instance=address_obj)
    teams = Team.objects.all()
    assignedto_list = request.POST.getlist('assigned_to')
    teams_list = request.POST.getlist('teams')
    if request.method == 'POST':
        form = ContactForm(request.POST, instance=contact_obj, assigned_to=users, account=clients)
